...
export default async function EditGalleryAlbum({ params }: { params: Promise<{ id: string }> }) {
  ...
  const addPhotos = addGalleryPhotos.bind(null, item.id);
  const saveOrder = reorderGalleryPhotos.bind(null, item.id);
  const removePhoto = removeGalleryPhoto.bind(null, item.id);
  const photos = images.map((image, index) => ({ id: image.id, src: `/api/media/${image.id}`, alt: `${item.title}, фото ${index + 1}` }));
  return <main className="admin-editor admin-edit-page"><div className="admin-edit-nav"><Link href="/admin">← К публикациям</Link><form action={togglePublished}><input type="hidden" name="id" value={item.id}/><input type="hidden" name="type" value="gallery"/><input type="hidden" name="published" value={String(!item.published)}/><button className="admin-publish">{item.published?"Снять с публикации":"Опубликовать"}</button></form></div><section className="admin-preview admin-project-preview">{images.length>0&&<div className="admin-preview-gallery">{images.map(image=><div key={image.id}><Image src={`/api/media/${image.id}`} alt="" fill sizes="380px" unoptimized/></div>)}</div>}<span>{item.published?"Опубликовано":"Черновик"} · {images.length} фотографий</span><h1>{item.title}</h1><p>Фотоархив за {item.year} год</p></section><section className="admin-media-manager"><h2>Фотографии альбома</h2><PhotoUploadForm action={addPhotos} maxFiles={50}/>{photos.length>0?<SortablePhotoGrid photos={photos} saveOrder={saveOrder} deletePhoto={removePhoto}/>:<p className="admin-empty">Фотографий пока нет.</p>}</section><h2>Редактирование</h2><form action={updateGalleryAlbum}><input type="hidden" name="id" value={item.id}/><label>Название альбома<input name="title" required defaultValue={item.title}/></label><label>Год<input name="year" type="number" min="2000" max="2100" required defaultValue={item.year}/></label><button type="submit">Сохранить изменения</button></form></main>;
}
